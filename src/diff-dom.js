(function () {
  ...
  var roughlyEqual = function roughlyEqual(e1, e2, uniqueDescriptors, sameSiblings, preventRecursion) {
    ...
    if (e1.nodeType === 3) {
      ...
      return preventRecursion ? true : e1.nodeValue === e2.nodeValue;
    }
    ...
    for (var i = e1.childNodes.length - 1; i >= 0; i--) {
      if (preventRecursion) {
        thesame = thesame && (e1.childNodes.item(i).nodeName === e2.childNodes.item(i).nodeName);
      } else {
        // note: we only allow one level of recursion at any depth. If 'preventRecursion'
        //       was not set, we must explicitly force it to true for child iterations.
        thesame = thesame && roughlyEqual(e1.childNodes.item(i), e2.childNodes.item(i), childUniqueDescriptors, true, true);
      }
    }
    return thesame;
  };
  ...
  var cleanCloneNode = function (node) {
    ...
    if (false/*node.nodeType != 8 && node.nodeType != 3*/) {
    ...
    if (node.nodeType === 3) {
      ...
      if (node.childNodes && node.childNodes.length > 0) {
        ...
        for (i = 0; i < node.childNodes.length; i++) {
          objNode[CHILD_NODES].push(nodeToObj(node.childNodes.item(i)));
        }
      }
      if (node.value) {
  ...
  var findCommonSubsets = function (c1, c2, marked1, marked2) {
    ...
    if (subsetsSame) {
      for (k = 0; k < len1; k++) {
        var c1Desc = elementDescriptors(c1.item(k));
        var c2Desc = elementDescriptors(c2.item(k));
  ...
  var findFirstInnerDiff = function (t1, t2, subtrees, route) {
    ...
    for (i = 0, last = shortest.length; i < last; i++) {
      if (gaps1[i] === true) {
        node = t1.childNodes.item(i);
        if (node.nodeType === 3) {
          if (t2.childNodes.item(i).nodeType === 3 && node.nodeValue != t2.childNodes.item(i).nodeValue) {
            testNode = node;
            while (testNode.nextSibling && testNode.nextSibling.nodeType === 3) {
              testNode = testNode.nextSibling;
              if (t2.childNodes.item(i).nodeValue === testNode.nodeValue) {
                similarNode = true;
                break;
              }
            }
            if (!similarNode) {
              ...
              k[OLD_VALUE] = node.nodeValue;
              k[NEW_VALUE] = t2.childNodes.item(i).nodeValue;
              return new Diff(k);
            }
          }
          k = {};
      ...
      if (gaps1[i] != gaps2[i]) {
        ...
        if (toGroup != i) {
          ...
          for (j = 0; j < group.length; j++) {
            if (!t1.childNodes.item(toGroup + j).isEqualNode(t1.childNodes.item(i + j))) {
  ...
  diffDOM.prototype = {
    ...
    findInnerDiff: function (t1, t2, route) {
      ...
      if (mappings === 0) {
        if (t1.nodeType === 3 && t2.nodeType === 3 && t1.nodeValue !== t2.nodeValue) {
          k = {};
          k[ACTION] = MODIFY_TEXT_ELEMENT;
          k[OLD_VALUE] = t1.nodeValue;
          k[NEW_VALUE] = t2.nodeValue;
          k[ROUTE] = route;
          return new Diff(k);
        }
      }
      ...
      if (mappings < 2) {
        ...
        for (i = 0, last = Math.max(t1.childNodes.length, t2.childNodes.length); i < last; i++) {
          e1 = t1.childNodes.item(i);
          e2 = t2.childNodes.item(i);
    ...
    applyDiff: function (tree, diff, diffOptions) {
      ...
      if (diff[ACTION] === ADD_ATTRIBUTE) {
        ...
        var group = diff[GROUP],
          ...
          to = diff[TO],
          child, reference;
        reference = node.childNodes.item(to + group.length);
        // slide elements up
        if (from < to) {
          for (var i = 0; i < group.length; i++) {
            child = node.childNodes.item(from);
            node.insertBefore(child, reference);
          }
        } else {
          // slide elements down
          reference = node.childNodes.item(to);
          for (var i = 0; i < group.length; i++) {
            child = node.childNodes.item(from + i);
            node.insertBefore(child, reference);
          }
        }
